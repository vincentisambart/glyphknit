...
- I am still a beginner at internationalization so the code might be full of mistakes.

Compared to using for example CoreText the goals are:
...
And another important goal for me is to improved my knowledge of Unicode and OpenType, understanding how things work.
...
Requirements:
- ninja (the build tool used by example for Chrome). To install it: brew install ninja
- ICU, FreeType, HarfBuzz: how-to-build-dependencies.txt quickly explains how I built those. I do have a script to build those for iOS but for the time being most of the development is using a one-architecture build for OS X.
...
- If you want to regenerate src/script_iterator-pairs.hh, you need a recent version (at least 1.9) of Ruby. Ruby 2.0 included in the last OS X works fine. Then just run the script. The needed data files are included in the repository (in data/UCD-7.0.0)
- If you want to regenerate src/language-data.hh, you also need a recent version of Ruby, but also the Nokogiri gem. To install it just run "gem install nokogiri". You also need to have a recent version of the CLDR (http://cldr.unicode.org/index/downloads), ICU4C (http://site.icu-project.org/repository), and lang-ietf-opentype (https://github.com/jclark/lang-ietf-opentype) repositories. I am using the very last trunk of all of them to generate the src/language-data.hh already there so you probably don't need to do it yourself.
